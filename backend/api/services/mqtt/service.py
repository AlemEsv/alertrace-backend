...
def connect(client, flags, rc, properties):
    logger.info(
        f"Connected to MQTT Broker: {settings.mqtt_broker}:{settings.mqtt_port}"
    )
...
async def message(client, topic, payload, qos, properties):
    try:
        ...
        if (
            len(topic_parts) >= 3
            and topic_parts[0] == "sensores"
            and topic_parts[2] == "data"
        ):
        ...
        if not device_id:
            logger.warning(
                f"Could not determine device_id from topic {topic} or payload"
            )
            return
