...
from database.models.database import Sensor, LecturaSensor, Alerta, ConfiguracionUmbral
import logging
...
def verify_and_generate_alerts(lectura: LecturaSensor, sensor: Sensor, db: Session):
    ...
    result = db.execute(
        select(ConfiguracionUmbral).where(
            ...
            ConfiguracionUmbral.activo == True,  # noqa: E712
        )
    )
    ...
    if not config:
        ...
        config = ConfiguracionUmbral(
            ...
            temp_min=15.0,
            temp_max=35.0,
            humedad_aire_min=40.0,
            humedad_aire_max=90.0,
            humedad_suelo_min=60.0,
            humedad_suelo_max=85.0,
            ph_min=6.0,
            ph_max=7.5,
            radiacion_min=0.0,
            radiacion_max=1000.0,
            activo=True,
        )
        db.add(config)
        db.commit()
    ...
    def check_metric(
        metric_value, min_val, max_val, metric_name, unit, msg_low, msg_high
    ):
        ...
        if metric_value < min_val:
            alerta = create_alert(
                sensor,
                metric_name,
                "high" if metric_name in ["temperatura", "ph"] else "medium",
                f"{msg_low}",
    ...
    check_metric(
        ...
        config.humedad_aire_min,
        config.humedad_aire_max,
        "humedad_aire",
        "%",
    ...
    check_metric(
        ...
        config.humedad_suelo_min,
        config.humedad_suelo_max,
        "humedad_suelo",
        "%",
        "🌧️ Humedad del Suelo Baja",
        "🌧️ Humedad del Suelo Alta",
    )
    check_metric(
        lectura.ph_suelo,
        config.ph_min,
    ...
    if (
        lectura.radiacion_solar is not None
        and lectura.radiacion_solar > config.radiacion_max
    ):
        alerta = create_alert(
            sensor,
            "radiacion",
            ...
            float(lectura.radiacion_solar),
            float(config.radiacion_max),
            db,
        )
        if alerta:
            alertas_generadas.append(alerta)
