...
def mock_supabase_and_jwt():
    with (
        patch("api.routes.auth.get_admin_client") as mock_get_admin,
        ...
        def sign_in_side_effect(payload):
            user = MockUser(id=test_user_id, email=payload["email"])
            return MockAuthResponse(
                user=user, session=MockSession(access_token="fake-jwt-token", user=user)
            )
...
def test_full_sensor_flow(client, mock_supabase_and_jwt):
    ...
    # 2. Login
    login_data = {"email": "farmer@example.com", "password": "SecurePassword123!"}
