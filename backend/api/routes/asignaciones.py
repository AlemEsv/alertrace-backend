...
router = APIRouter(tags=["Asignaciones Sensor-Trabajador"])
...
async def assign_sensor(
    ...
    result_trabajador = await db.execute(
...
async def unassign_sensor(
    ...
    result_asignacion = await db.execute(
        select(AsignacionSensor).where(AsignacionSensor.id_asignacion == asignacion_id)
    )
...
async def list_worker_assignments(
    ...
    for asig in asignaciones:
        result_sensor = await db.execute(
            select(Sensor).where(Sensor.id_sensor == asig.id_sensor)
        )
        sensor = result_sensor.scalars().first()
        resultado.append(
            {
...
async def list_sensor_assignments(
    ...
    # Consultar asignaciones
    query = select(AsignacionSensor).where(AsignacionSensor.id_sensor == sensor_id)
    ...
    for asig in asignaciones:
        result_trabajador = await db.execute(
            select(Trabajador).where(Trabajador.id_trabajador == asig.id_trabajador)
        )
        trabajador = result_trabajador.scalars().first()
        resultado.append(
            {
...
async def list_all_assignments(
    ...
    result_sensores = await db.execute(
        select(Sensor.id_sensor).where(Sensor.id_empresa == current_user.id_empresa)
    )
    ...
    query = select(AsignacionSensor).where(AsignacionSensor.id_sensor.in_(sensor_ids))
