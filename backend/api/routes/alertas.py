...
async def get_alerts(
    ...
    try:
        ...
        """),
            {"empresa_id": empresa_id, "limit": limit, "skip": skip},
        )
            ...
            alertas_response.append(
                {
                    ...
                    "mensaje": alerta[5],
                    "resuelta": alerta[6],
                    "fecha_creacion": alerta[7].isoformat()
                    if alerta[7]
...
async def get_alert(
    alerta_id: int, db: DbSession, current_user: Trabajador = Depends(get_current_user)
):
    """Obtener una alerta específica"""
    result = await db.execute(
        select(Alerta).where(
            Alerta.id_alerta == alerta_id, Alerta.id_empresa == current_user.id_empresa
        )
    )
...
async def resolve_alert(
    ...
    result_sensor = await db.execute(
        select(Sensor).where(Sensor.id_sensor == alerta.id_sensor)
    )
    ...
    if sensor:
        ...
        if ultima_lectura:
            result_umbral = await db.execute(
                select(ConfiguracionUmbral).where(
            ...
            if umbral:
                ...
                if alerta.tipo_alerta == "temperatura":
                    if ultima_lectura.temperatura and (
                        ultima_lectura.temperatura < umbral.temp_min
                        or ultima_lectura.temperatura > umbral.temp_max
                    ):
                    ...
                    if ultima_lectura.ph_suelo and (
                        ultima_lectura.ph_suelo < umbral.ph_min
                        or ultima_lectura.ph_suelo > umbral.ph_max
                    ):
                        problema_persiste = True
                    ...
                    if ultima_lectura.humedad_suelo and (
                        ultima_lectura.humedad_suelo < umbral.humedad_suelo_min
                        or ultima_lectura.humedad_suelo > umbral.humedad_suelo_max
                    ):
                        problema_persiste = True
                    ...
                    if (
                        ultima_lectura.radiacion_solar
                        and ultima_lectura.radiacion_solar > 1000
                    ):
                        problema_persiste = True
...
async def get_pending_alerts_count(
    ...
    result = await db.execute(
        ...
        .where(
            ...
            Alerta.resuelta == False,  # noqa: E712
        )
    )
    count = result.scalar()
