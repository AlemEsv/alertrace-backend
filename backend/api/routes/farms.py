...
async def create_farm(
    ...
    farm = Farm(id_empresa=empresa.id_empresa, **farm_data.model_dump())
    db.add(farm)
    await db.commit()
...
async def get_farm(
    farm_id: int, db: DbSession, empresa: Empresa = Depends(get_current_empresa)
):
    """Obtener detalles de una finca específica"""
    result = await db.execute(
        select(Farm).where(
            Farm.id_farm == farm_id, Farm.id_empresa == empresa.id_empresa
        )
    )
...
async def delete_farm(
    ...
    result = await db.execute(
        select(Farm).where(Farm.id == farm_id, Farm.id_empresa == empresa.id_empresa)
    )
    farm = result.scalars().first()
...
@router.post(
    "/{farm_id}/certifications",
    response_model=FarmCertificationResponse,
...
async def list_certifications(
    ...
    if active_only:
        query = query.where(FarmCertification.active == True)  # noqa: E712
...
async def update_certification(
    ...
    result = await db.execute(
        select(FarmCertification)
        .join(Farm)
        .where(FarmCertification.id == cert_id, Farm.id_empresa == empresa.id_empresa)
    )
...
async def delete_certification(
    cert_id: int, db: DbSession, empresa=Depends(get_current_empresa)
):
