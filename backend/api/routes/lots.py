...
async def create_lot(
    lot_data: LotCreate, db: DbSession, empresa=Depends(get_current_empresa)
):
    """Crear nuevo lote de productos"""
    if lot_data.id_farm:
        result_farm = await db.execute(
            select(Farm).where(
                Farm.id == lot_data.id_farm, Farm.id_empresa == empresa.id_empresa
            )
        )
        ...
        if not farm:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Finca no encontrada"
            )
    ...
    lot = Lot(id_empresa=empresa.id_empresa, **lot_data.model_dump())
    db.add(lot)
    await db.commit()
...
async def list_lots(
    ...
    result = await db.execute(
        query.order_by(desc(Lot.created_at)).offset(skip).limit(limit)
    )
...
async def get_lot(lot_id: int, db: DbSession, empresa=Depends(get_current_empresa)):
    """Obtener detalles de un lote específico"""
    result = await db.execute(
        select(Lot).where(Lot.lot_id == lot_id, Lot.id_empresa == empresa.id_empresa)
    )
...
async def get_lot_traceability(
    lot_id: int, db: DbSession, empresa=Depends(get_current_empresa)
) -> Dict[str, Any]:
    ...
    result_harvest = await db.execute(
        select(HarvestEvent)
        .where(HarvestEvent.lot_id == lot_id)
        .order_by(HarvestEvent.event_time)
    )
    harvest_events = result_harvest.scalars().all()

    result_processing = await db.execute(
        select(ProcessingEvent)
        .where(ProcessingEvent.lot_id == lot_id)
        .order_by(ProcessingEvent.event_time)
    )
    processing_events = result_processing.scalars().all()

    result_transfer = await db.execute(
        select(TransferEvent)
        .where(TransferEvent.lot_id == lot_id)
    ...
    return {
        ...
        "harvest_events": [
            HarvestEventResponse.model_validate(e) for e in harvest_events
        ],
        "processing_events": [
            ProcessingEventResponse.model_validate(e) for e in processing_events
        ],
        "transfer_events": [
            TransferEventResponse.model_validate(e) for e in transfer_events
        ],
        "total_events": len(harvest_events)
...
@router.post(
    "/{lot_id}/harvest",
    response_model=HarvestEventResponse,
...
@router.post(
    "/{lot_id}/processing",
    response_model=ProcessingEventResponse,
    status_code=status.HTTP_201_CREATED,
)
...
@router.post(
    "/{lot_id}/transfer",
    response_model=TransferEventResponse,
    status_code=status.HTTP_201_CREATED,
)
