...
async def create_sensor(
    sensor_data: SensorCreate, db: DbSession, current_user: CurrentUser
):
    """
    ...
    try:
        ...
        return {
            ...
            "sensor": {
                ...
                "latitud": float(nuevo_sensor.latitud)
                if nuevo_sensor.latitud
                else None,
                "longitud": float(nuevo_sensor.longitud)
                if nuevo_sensor.longitud
                else None,
            },
        }
...
async def get_sensors(
    db: DbSession, current_user: CurrentUser, activo: Optional[bool] = None
):
    ...
    if isinstance(current_user, Trabajador):
        ...
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Tipo de usuario no válido"
        )

    if activo is not None:
        query = query.where(
            Sensor.estado == ("activo" if activo else "inactivo")
        )  # Assuming 'activo'/'inactivo' strings based on other code
...
async def get_sensors_with_readings(
    ...
    for sensor in sensores:
        result_lectura = await db.execute(
            select(LecturaSensor)
            .where(LecturaSensor.id_sensor == sensor.id_sensor)
            .order_by(LecturaSensor.timestamp.desc())
            .limit(1)
        )
        ...
        if lectura_reciente:
            sensor_data.update(
                {
                    "temperatura": round(float(lectura_reciente.temperatura), 1)
                    if lectura_reciente.temperatura is not None
                    else 0.0,
                    "humedad_aire": round(float(lectura_reciente.humedad_aire), 1)
                    if lectura_reciente.humedad_aire is not None
                    else 0.0,
                    "humedad_suelo": round(float(lectura_reciente.humedad_suelo), 1)
                    if lectura_reciente.humedad_suelo is not None
                    else 0.0,
                    "ph": round(float(lectura_reciente.ph_suelo), 1)
                    if lectura_reciente.ph_suelo is not None
                    else 7.0,
                    "radiacion_solar": round(float(lectura_reciente.radiacion_solar), 1)
                    if lectura_reciente.radiacion_solar is not None
                    else 0.0,
...
async def update_sensor(
    sensor_id: int, sensor_data: SensorUpdate, db: DbSession, current_user: CurrentUser
):
    ...
    if hasattr(current_user, "id_empresa"):
        ...
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Usuario sin empresa asignada"
        )
    ...
    try:
        ...
        user_id = getattr(
            current_user,
            "id_trabajador",
            getattr(current_user, "id_empresa", "unknown"),
        )
        logger.info(
...
async def move_sensors(
    move_data: SensorMoveRequest, db: DbSession, current_user: CurrentUser
):
    """
    ...
    try:
        ...
        for sensor in sensores:
            try:
                ...
                errors.append({"sensor_id": sensor.id_sensor, "error": str(e)})
...
def receive_sensor_data(data: SensorData, db: Session = Depends(get_sync_db)):
    """
