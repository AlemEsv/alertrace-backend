...
from api.models import Token, UserProfile, RegisterRequest
...
async def register_user(request: RegisterRequest, db: DbSession):
    ...
    if request.user_type == "empresa":
        if not request.nombre_empresa or not request.ruc:
            raise HTTPException(
                status_code=400,
                detail="Nombre de empresa y RUC son requeridos para cuentas de empresa",
            )
        ...
        if not request.nombres or not request.apellidos or not request.dni:
            raise HTTPException(
                status_code=400, detail="Nombres, apellidos y DNI son requeridos"
            )
    ...
    try:
        ...
        async with db.begin():
            ...
            result = await db.execute(
                select(Empresa).where(Empresa.ruc == empresa_data["ruc"])
            )
            ...
            if existing_empresa:
                ...
                raise HTTPException(
                    status_code=400,
                    detail="Una empresa con este RUC ya está registrada.",
                )
            ...
            logger.info(
                f"Trabajador admin creado: {request.email}, empresa: {nueva_empresa.id_empresa}"
            )
...
async def login(credentials: LoginRequest, db: DbSession):
    ...
    try:
        ...
        user_id = (
            uuid.UUID(str(supabase_user.id))
            if supabase_user and supabase_user.id
            else None
        )

        # Verificar que el trabajador exista en la base de datos
        result = await db.execute(
            select(Trabajador).where(Trabajador.user_id == user_id)
        )
        ...
        if not trabajador_existente:
            ...
            logging.warning(
                f"Usuario autenticado pero sin perfil de trabajador: {credentials.email}"
            )
            raise HTTPException(
...
async def read_users_me(credentials: AuthCreds, db: DbSession):
    """
    ...
    try:
        ...
        result = await db.execute(
            select(Trabajador).where(Trabajador.user_id == user_id_uuid)
        )
        ...
        result_empresa = await db.execute(
            select(Empresa).where(Empresa.id_empresa == perfil_trabajador.id_empresa)
        )
        empresa_asociada = result_empresa.scalars().first()

        user_type = (
            "empresa" if perfil_trabajador.rol == "admin_empresa" else "trabajador"
        )
        ...
        user_profile_data = {
            ...
            "empresa_nombre": empresa_asociada.razon_social
            if empresa_asociada
            else "Sin empresa asignada",
            "user_type": user_type,
        }
